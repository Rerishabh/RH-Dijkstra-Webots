...
# Change ALPHA manually here for each trial:
# 0.0 = Risk-Disabled Shortest-Path Baseline
# 1.0 = Linear Risk Sensitivity
# 2.0 = Risk-Aware Reference Configuration
...
while robot.step(timestep) != -1:
    ...
    # Pure Physical Step Increment Logic
    ...
    if counter % 20 == 0:
        ...
        if math.sqrt((goal_pos[0] - epuck_pos[0])**2 + (goal_pos[1] - epuck_pos[1])**2) < 0.25:
            ...
            # Write metrics directly to data/sensitivity_study.csv
            csv_path = os.path.join(DATA_DIR, "sensitivity_study.csv")
        ...
        if path and len(path) > 1:
            ...
            message = f"{target_x} {target_y} {ALPHA}"
        else:
            message = f"STOP {ALPHA}"
