...
To run this simulation setup locally on your machine, follow these steps:
...
### 2. Launch Webots
Open **Webots R2025a** (or a newer compatible version).

### 3. Open the Simulation World
Navigate to:
...
- Pre-configured grid topology
- Dynamic obstacle trajectories
- Controller bindings
- Simulation parameters

### 4. Tune Risk Sensitivity
Open:
...
Modify the global `ALPHA` parameter according to the desired risk sensitivity:

```python
ALPHA = 0.0   # Classical shortest path baseline
ALPHA = 1.0   # Standard risk-aware routing
ALPHA = 2.0   # Paper default configuration
ALPHA = 3.0   # Highly conservative route selection
```

### 5. Run the Simulation
Click the green **Play** button in Webots.

After the robot reaches the goal region, the supervisor automatically logs the experimental metrics into:
